;(function($) {
  ...
  function Plugin(option) {
    return this.each(function() {
      ...
      if (!data)
        $this.data(
          'lgb.SliderCaptcha',
          (data = new SliderCaptcha(this, options))
        )
  ...
  _proto.initImg = function() {
    ...
    img.setSrc = function() {
      ...
      if (!src || src === '')
        src = require('@/assets/images/captures/' +
          arr[Math.round(Math.random() * 22)] +
          '.jpg')
      if (isIE) {
        //
        var xhr = new XMLHttpRequest()
        xhr.onloadend = function(e) {
          var file = new FileReader() // FileReader
  ...
  _proto.bindEvents = function() {
    ...
    var handleDragEnd = function(e) {
      ...
      that.trail = trail
      var { spliced, verified } = that.verify()
      if (spliced && verified) {
